...
import { findCanonicalMatchForKoreaJapan2002Entry } from "./koreaJapan2002-replay-apply";
import {
  KOREA_JAPAN_2002_HIGHLIGHTS_CATALOG,
  KOREA_JAPAN_2002_HIGHLIGHTS_VERIFIED_BY,
  type KoreaJapan2002HighlightsCatalogEntry,
...
export type KoreaJapan2002HighlightsMapping = {
  entry: (typeof KOREA_JAPAN_2002_HIGHLIGHTS_CATALOG)[number];
  match: CanonicalMatch;
};

export { officialHighlightsLabel, officialHighlightsNotes };

export function mapKoreaJapan2002HighlightsCatalog(
  matches: CanonicalMatch[]
): KoreaJapan2002HighlightsMapping[] {
  return KOREA_JAPAN_2002_HIGHLIGHTS_CATALOG.map((entry) => ({
    entry,
...
function buildHighlightSource(
  match: CanonicalMatch,
  entry: Pick<
    KoreaJapan2002HighlightsCatalogEntry,
  ...
  return {
    ...
    humanVerification: {
      status: "verified",
      verifiedBy: KOREA_JAPAN_2002_HIGHLIGHTS_VERIFIED_BY,
      verifiedAt,
...
export function applyKoreaJapan2002HighlightsCatalogSource(
  match: CanonicalMatch,
  entry: Pick<
    KoreaJapan2002HighlightsCatalogEntry,
    "url" | "provider" | "packageKind"
...
export function applyKoreaJapan2002HighlightsCatalogToArchive(
  matches: CanonicalMatch[],
  verifiedAt: string = new Date().toISOString()
): { matches: CanonicalMatch[]; mappings: KoreaJapan2002HighlightsMapping[] } {
  const mappings = mapKoreaJapan2002HighlightsCatalog(matches);
  const byId = new Map(
  ...
  for (const { entry, match } of mappings) {
    ...
    byId.set(
      ...
      applyKoreaJapan2002HighlightsCatalogSource(current, entry, verifiedAt)
    );
  }
